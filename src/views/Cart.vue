...
<script>
  ...
  export default{
    ...
    filters:{
      //currency具名函数格式化金额，所以上面引用需要{currency}，不能直接currency
      currency:currency
    },
    computed:{
      checkAllFlag(){
    ...
    methods:{
      ...
      editCart(flag,item){
        if(flag=='add'){
          ...
          item.checked = item.checked=="1"?'0':'1'; //取反选中和取消选中
        }
      ...
      //选择框
      toggleCheckAll(){
        //没有办法赋值给原来的checkAllFlag，因为computed里一监视发生变化就会重新取反
